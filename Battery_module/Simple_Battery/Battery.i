...
[ScalarKernels]
    ...
    [RHS_Battery_turbine]
      type = ParsedODEKernel
      variable = T_battery
      expression = '-cp_r*mass_flow_rate_reactor*(T_from_turbine-T_to_turbine)/Cb'
      constant_expressions = '${fparse Cb} ${fparse cp_r} ${fparse mass_flow_rate_reactor}'
      constant_names = 'Cb cp_r mass_flow_rate_reactor'
...
[Postprocessors]
 ...
 # change the MFR currently set at 10
   type = ParsedPostprocessor 
   expression = '(T_to_turbine-500)*cp_r*mass_flow_rate_reactor'
   constant_expressions = '${fparse cp_r} ${fparse mass_flow_rate_reactor}'
   constant_names = 'cp_r mass_flow_rate_reactor'
   pp_names = 'T_to_turbine'
   execute_on = 'INITIAL TIMESTEP_BEGIN TIMESTEP_END'
 []
 [Energy_to_turbine]
